...
public class GalleryActivity
    ...
    public static Intent makeStartIntent(
        ...
        Intent newIntent = new Intent(context, GalleryActivity.class);
        ...
        // TODO - you fill in here.
        newIntent.putParcelableArrayListExtra(INTENT_EXTRA_URLS,inputUrls);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            super.setItems(extractInputUrlsFromIntent(makeStartIntent(this, urlList)));
    ...
    private List<Uri> extractInputUrlsFromIntent(Intent intent) {
        ...
        ArrayList<Uri> validateURLList = new ArrayList<>();
        validateURLList = intent.getParcelableArrayListExtra(INTENT_EXTRA_URLS);

        if(validateInput(validateURLList) == true)
        {
    ...
    private boolean validateInput(ArrayList<Uri> inputUrls) {
        ...
        boolean invalidUrlFlag = false;
        if(inputUrls == null)
        {
            ViewUtils.showToast(this,R.string.input_url_list_is_null,null);
        }
        else if(inputUrls.size() == 0)
        {
            ViewUtils.showToast(this,R.string.input_url_list_is_empty,null);
        }
        else
        {
            for(Uri i : inputUrls)
            {
                if(URLUtil.isValidUrl(i.toString()) == false)
                {
                    invalidUrlFlag = true;
                    break;
                }
